...
class TypeParser:
    """Parser for type definitions supporting both syntactic sugar and detailed JSON formats."""
    ...
    _FLOAT_PATTERN = re.compile(r'^float\s*\(\s*(-?\d+)\s*,\s*(-?\d+)\s*\)$', re.IGNORECASE)
    _DATETIME_PATTERN = re.compile(r'^datetime\s*\(\s*[\'"](.+?)[\'"]\s*\)$', re.IGNORECASE)
    _SIMPLE_TYPE_PATTERN = re.compile(r'^(string|str|integer|int|float|boolean|bool|date|datetime)$', re.IGNORECASE)

    @classmethod
    def parse_type_definition(cls, type_def: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        Returns:
            ...
            - Additional metadata keys based on type (max_length, precision, scale, format)
        ...
        if isinstance(type_def, dict):
            ...
            raise TypeParseError(f"Type definition must be string or dict, got {type(type_def)}")
    ...
    def _parse_detailed_format(cls, type_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            "type": cls._SUPPORTED_TYPES[type_name].value
        }
    ...
    def _validate_metadata(cls, parsed_type: Dict[str, Any]) -> None:
        ...
        if "max_length" in parsed_type:
            if type_value != DataType.STRING.value:
                raise TypeParseError(
                    f"max_length can only be specified for STRING type, not {type_value}"
                )
            if not isinstance(parsed_type["max_length"], int) or parsed_type["max_length"] <= 0:
        ...
        if "precision" in parsed_type or "scale" in parsed_type:
            if type_value != DataType.FLOAT.value:
                raise TypeParseError(
                    f"precision/scale can only be specified for FLOAT type, not {type_value}"
                )
                
        if "precision" in parsed_type:
            if not isinstance(parsed_type["precision"], int) or parsed_type["precision"] <= 0:
        ...
        if "scale" in parsed_type:
            ...
            if "precision" in parsed_type and parsed_type["scale"] > parsed_type["precision"]:
    ...
    def is_syntactic_sugar(cls, type_def: Union[str, Dict[str, Any]]) -> bool:
        ...
        return bool(
            cls._STRING_PATTERN.match(type_str) or
            cls._FLOAT_PATTERN.match(type_str) or
            cls._DATETIME_PATTERN.match(type_str) or
            cls._SIMPLE_TYPE_PATTERN.match(type_str)
        )

    @classmethod
    def normalize_to_detailed_format(cls, type_def: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if "type" in parsed:
            ...
            parsed["expected_type"] = parsed["type"]  # For schema executor
