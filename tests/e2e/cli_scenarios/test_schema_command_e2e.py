    ...
    rules = {
        ...
        "orders": {
            "rules": [
                ...
                {"field": "product_name", "type": "string", "max_length": 255, "required": True},
                {"field": "quantity", "type": "integer", "required": True},
                {"field": "price", "type": "float", "precision": 10, "scale": 2, "required": True},
                {"field": "status", "type": "string", "max_length": 50, "required": True},
...
@pytest.mark.parametrize("db_url", _param_db_urls())
def test_multi_table_schema_metadata_validation_failures(tmp_path: Path, db_url: str) -> None:
    ...
    rules = {
        "customers": {
            "rules": [
                {"field": "id", "type": "integer", "required": True},
                {"field": "name", "type": "string", "max_length": 10},  # Too restrictive - DB has VARCHAR(255)
                {"field": "email", "type": "integer"},  # Wrong type - DB has VARCHAR(255)
        ...
        "orders": {
            "rules": [
                {"field": "id", "type": "integer", "required": True},
                {"field": "customer_id", "type": "string"},  # Wrong type - DB has INTEGER
                {"field": "product_name", "type": "string", "max_length": 10},  # Too restrictive - DB has VARCHAR(255)
                {"field": "quantity", "type": "float"},  # Wrong type - DB has INTEGER
                {"field": "price", "type": "float", "precision": 5, "scale": 4},  # Inconsistent - DB has DECIMAL(10,2)
                {"field": "status", "type": "string", "max_length": 5},  # Too restrictive - DB has VARCHAR(50)
    ...
    failed_fields = [f for f in fields if any(
        (check.get("status") == "FAILED" or check.get("failure_code") == "METADATA_MISMATCH")
        for check in f.get("checks", {}).values() if isinstance(check, dict)
    )]
