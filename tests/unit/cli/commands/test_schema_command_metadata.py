...
from shared.enums import RuleType, ConnectionType
...
class TestSchemaCommandMetadataParsing:
    ...
    def test_valid_metadata_string_length_parsing(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        schema_file = write_temp_file(tmp_path, "schema.json", json.dumps(schema_content))
        
        # Mock the entire schema command execution to avoid validation issues
        captured_rules = []
        
        def mock_decompose(payload, source_config):
        ...
        class MockValidator:
            def __init__(self, source_config: Any, rules: Any, core_config: Any, cli_config: Any):
                self.rules = rules  # Store for later verification
                
            async def validate(self) -> List[ExecutionResultSchema]:
                return []
        
        monkeypatch.setattr("cli.commands.schema._decompose_schema_payload", mock_decompose)
    ...
    def test_valid_metadata_float_precision_parsing(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    def test_backward_compatibility_without_metadata(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
class TestSchemaCommandRuleDecomposition:
    ...
    def test_metadata_included_in_schema_rule_parameters(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
...
class TestSchemaCommandErrorHandling:
    ...
    def test_malformed_json_with_metadata(self, tmp_path: Path):
    ...
    def test_missing_required_fields_with_metadata(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
        ...
        data_path = write_temp_file(tmp_path, "data.csv", "id\n1\n")
        schema_file = write_temp_file(tmp_path, "schema.json", json.dumps(incomplete_content))
